...
void main(void)
{
    ...
    IX = &X; /* Ix aputa a x. Ix tiene la direccion de x */
...
printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
IX = IX + 4; /* IX se modifica. oberseva que el apuntador se mueve 4 posiciones y cae en una direccion afuera del arreglo. Esto ocasionara un error. */
...
IX = IX + 1; /*IX se muevbe una posicion y cae en una celda incorrecta. */
